...
Payload ASLA GitHub access token TAŞIMAZ (#79 kuralı — token yalnız callback
içinde `/user` çağrısı için kullanılır, sonra atılır) — yalnız
`{"handle": str, "avatar_url": str | None}`.
...
def sign_session(secret: str, *, handle: str, avatar_url: str | None) -> str:
    payload = json.dumps(
        {"handle": handle, "avatar_url": avatar_url}, separators=(",", ":")
    ).encode("utf-8")
    payload_b64 = _b64_encode_nopad(payload)
...
def verify_session(secret: str, cookie_value: str) -> dict:
    ...
    if not isinstance(payload, dict) or "handle" not in payload:
        raise SessionSignatureError("çerez gövdesinde handle yok")
    return payload
