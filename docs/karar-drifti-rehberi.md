...
**O gün ölçülen durum:** `Fly/Render` ikilisi doküman metninde hâlâ **12 kez** "açık" duruyordu — `.env.example`'da *"Hosted platform secret seti (#186 — S3 go-live boşluk analizi): Fly/Render'a…"* ve `docs/sprint3-kontratlar.md`'da *"Fly/Render secret'ları env-string; mount'lu dosya değil…"* cümleleri dâhil (commit `5cf2009`'daki hâliyle doğrulandı — `git show 5cf2009:.env.example` satır 18/45, `git show 5cf2009:docs/sprint3-kontratlar.md` satır 69; **satır numarası yerine alıntı kullanılıyor** çünkü bu iki dosya D-46 self-host geçişiyle tekrar değişti — bugün `.env.example`'da Fly/Render referansı hiç YOK, `docs/sprint3-kontratlar.md`'da yalnızca geçmişe atıfla *"eski Fly/Render gibi"* kalıntısı var, #257 bulgu 5 / #313 — kaymaya dayanıklı referans önerisi buradan) — ama uygulama yüzeyinde **Render'a ait tek bir artefakt yoktu**: ne `render.yaml`, ne `render.com`, ne bir issue, ne bir satır. Fly ise `eval/datasets/scope-corpus.json`'daki kapsam metnine kadar girmişti — yani geri alma, **eval ground-truth'unu** yeniden etiketlemek demekti.
