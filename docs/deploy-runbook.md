...
Bu turda referans alınan (henüz `main`'e inmemiş, ayrı dallarda ilerleyen) işler açıkça **"hedef durum"** olarak işaretlendi — bugünkü gerçek davranışla karıştırılmasın diye (bkz. §8 ve ilgili notlar):

| Bağımlı iş | Bugünkü durum | Bu runbook'taki karşılığı |
|---|---|---|
| **#189** — `scripts/smoke.py` + `make smoke` | ayrı dalda (`T-189-live-smoke`), henüz `main`'de değil | §4 — komut bugün Makefile'da yok; landed olunca aynen çalışır |
| **#192** — `.github/workflows/deploy.yml` (CD) | ayrı dalda (`T-192-deploy-cd-fly`), henüz `main`'de değil | §3 adım 10 |
| **#63** — `DEMO_MODE` hosted sertleştirme | ayrı dalda (`T-63-hosted-demo-hardening`), henüz `main`'de değil | §6 |
| **#187** — `release_command` (alembic-on-deploy) | henüz yok, `fly.toml` başlığı bunu açıkça diğer bir işe devretmiş | §3 adım 6 |
| **#182/#183** — self-host pgvector app + hosted DI | D-39 kararı Ek C3'te donuk; sağlayıcı kurulumu ayrı iş | §3 adım 2–3 |
...
└───────────────┘   (A2 çift-yön) └────────────┬────────────┘                      └──────────────────┘
                                               │
                          GitHub Actions ──────┘  (workflow_run: CI yeşil → flyctl deploy, #192)
                          CI → lint+test+eval-gate → Deploy (Fly)
```
...
- **CD (GitHub Actions → `deploy.yml`, #192):** yalnız `main`'e **push** + CI (`lint-test` + `eval-gate`) **yeşil** olunca `flyctl deploy` koşar; PR/fork çalıştırmaz (fail-safe token kapısı, §3 adım 10).

---

## 2. Env → platform eşleme tablosu

`.env.example`'daki **31 anahtarın tamamı** aşağıda. *(30'u düz `ANAHTAR=` satırı, 1'i — `CORS_ORIGINS` — yalnız yorum-satırı örneği; ikisi de sayılır, bkz. dosyanın kendisi.)* Dört sınıf: **Fly secret** (şifreli, `fly secrets set`) · **Vercel env** (build-time, Vercel dashboard) · **yalnız-local** (hiçbir platforma girilmez) · **CI secret** (`gh secret set`, pipeline).
...
| 20 | `GITHUB_REPO_NAME` | Fly secret | `fly secrets set` | izlenen repo adı — **#63 landed olunca**: `DEMO_MODE=true` iken bu ikisi eksikse uygulama **hiç açılmaz** (fail-closed, §6) |
...
| 29 | `VITE_API_BASE_URL` | **Vercel env** (build-time) | Vercel dashboard → *Environment Variables* → Production | = Fly backend public URL (A2 çift-yön); **origin-only** — query/hash `lib/config.ts` tarafından reddedilir; değişince **redeploy şart** (build-time gömülür, "Save" yetmez) |
...
| Anahtar | Nerede | Not |
|---|---|---|
| `PORT` | Fly `[env]` (commit'li) | `fly.toml`'da `"8000"` — Fly bunu kendi enjekte eder, `.env.example`'a girmez (yalnızca platform sözleşmesi) |
...
| `DEMO_MODE` + 6× `DEMO_*` | Fly `[env]` / Fly secret (opsiyonel) — **hedef, #63 henüz `main`'de değil** | §6'da ayrıntı; landed olunca `fly.toml [env] DEMO_MODE="true"` + `.env.example`'a eklenecek satırlar |
...
`grup54-db` için **ayrı** bir Fly manifesti gerekir (imaj: `pgvector/pgvector:pg16`, volume mount `/var/lib/postgresql/data`). **`[http_service]` / public port EKLEME** — erişim yalnız Fly'ın özel ağı (`.internal:5432`) üzerinden olacak, dışarıya hiç açılmayacak.
...
GitHub App → Webhook URL: `https://<fly-app>.fly.dev/webhook`. `GITHUB_WEBHOOK_SECRET`'ı **rotate et** (yeni değer yalnız Fly secret'ta yaşar, hiçbir kanalda paylaşılmaz).
...
**Hedef durum — `make smoke` (#189, henüz `main`'de değil):** landed olunca komut şu şekilde çalışacak:
...
**#189 `main`'e inene kadar** manuel eşdeğeri:
...
## 6. Hosted demo sertleştirme (#63 — hedef durum, henüz `main`'de değil)

`T-63-hosted-demo-hardening` dalı `DEMO_MODE` bayrağını + altı `DEMO_*` ayarını ekliyor (rate cap, cached-verdict). Landed olunca `fly.toml [env]`'e `DEMO_MODE = "true"` eklenecek ve bu bölüm bağlayıcı hale gelir:
...
| `/health` → `gemini: "missing"` (anahtar set edilmiş olmasına rağmen) | (a) `GEMINI_API_KEY` gerçekten yok, **ya da** (b) `DEMO_MODE` (#63) judge'ı sarmalıyor (R1, §6) | (a) `fly secrets list` ile digest kontrolü; (b) landed olursa bilinen sınır — bkz. §6 |
...
| Uygulama Fly'da **hiç açılmıyor**, health-check sürekli kırmızı | (#63 landed sonrası) `DEMO_MODE=true` ama `GITHUB_REPO_OWNER`/`GITHUB_REPO_NAME` eksik — fail-closed açılış | İkisini de `fly secrets set` ile ekle, yeniden deploy et |
