...
> ✅ **Çapraz etkileşim R1 — KAPANDI (#63).** Bir ara `DEMO_MODE=true` iken judge, `health.py`'nin `isinstance(..., GeminiJudgeAdapter)` kontrolünden geçmeyen bir sarmalayıcıya (`CachedConflictJudge`) sarılıyordu → hosted `/health` **her zaman** `gemini: "missing"` derdi ve `make smoke` (hosted'da otomatik strict) **kalıcı** `exit 1` olurdu. #63 bunu kendi dalında düzeltti: `health.py` artık `getattr(judge_port, "inner", judge_port)` ile sarmalayıcıyı açıyor, `test_health_demo_modda_gercek_wiring_uzerinden_gemini_configured_raporlar` regresyon testiyle kilitli. Yani `DEMO_MODE=true` iken `/health` Gemini'yi **doğru** raporlar ve smoke kapısı bu yüzden kırmızı kalmaz. `SMOKE_STRICT=0` ile smoke'u boyamak yine **yasak** (§4 kuralı).
...
| `/health` → `gemini: "missing"` | **`GEMINI_API_KEY` gerçekten yok / makineye ulaşmamış.** (Eskiden ikinci bir olası neden vardı — `DEMO_MODE` sarmalayıcısı, R1 — ama #63 onu kapattı; §6.) Yani bu belirti artık **tek anlama gelir: gerçek bir yapılandırma eksiği.** "Bilinen sınır" diye geçme. | `fly secrets list -a ensemble-backend` ile `GEMINI_API_KEY` digest'i var mı bak; yoksa `fly secrets set` + makine yeniden başlasın (secret set etmek deploy tetikler), sonra `curl .../health` ile teyit et |
