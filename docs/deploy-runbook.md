...
| 39d | `ENSEMBLE_SERVE_FRONTEND` | sunucu env dosyası (opsiyonel) | — | kod default `false` — backend frontend'i kendisi servis etsin mi (tek süreçli masaüstü paketi). **Açık bayrak, otomatik tespit değil:** eski "`dist/` varsa mount et" davranışı uygulamanın yollarını "birinin `npm run build` çalıştırıp çalıştırmadığına" bağlıyordu (CI'da 404, geliştirici makinesinde SPA — iki test bu yüzden yerelde kırmızıydı) |
